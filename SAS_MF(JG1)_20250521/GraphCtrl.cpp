...
#define UM_OSCOPEPOSITION			1234

/////////////////////////////////////////////////////////////////////////////
// CGraphCtrl

CFont	CGraphCtrl::sm_fontAxis;
LOGFONT	CGraphCtrl::sm_logFontAxis;
...
CGraphCtrl::CGraphCtrl(int NTrends)
{
	int i;
...
/// ADD		
	...
	// G.Hayduk: actually I needed an GraphCtrl to draw specific number of
	// data samples and stretch them on the plot ctrl. Now, GraphCtrl has
...
BOOL CGraphCtrl::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID) 
{
	BOOL result;
	...
	ready = true;
//	printf("\nCGraphCtrl::nCreate() %d %s",__LINE__,__FILE__);
	return result;
}
...
//
void CGraphCtrl::SetTrendRatio(int iTrend, UINT iRatio)
{
...
void CGraphCtrl::SetLegendLabel(CString string, int iTrend)
{
	m_PlotData[iTrend].LegendLabel = string;
	InvalidateCtrl(false);
}

void CGraphCtrl::SetBarsPlot(bool BarsPlot, int iTrend)
{
	m_PlotData[iTrend].BarsPlot = BarsPlot;
	InvalidateCtrl(false);
}

void CGraphCtrl::SetRange(double dLower, double dUpper, int iTrend)
{
...
void CGraphCtrl::SetRanges(double dLower, double dUpper)
{
	ASSERT( dUpper > dLower );		// error 표시 하기위하여
...
void CGraphCtrl::SetXUnits(CString string, CString XMin, CString XMax)
{
	m_str.XUnits = string;
	m_str.XMin = XMin;
	m_str.XMax = XMax;
...
void CGraphCtrl::SetYUnits(CString string, CString YMin, CString YMax)
{
	m_str.YUnits = string;
	m_str.YMin = YMin;
	m_str.YMax = YMax;
...
void CGraphCtrl::SetPlotColor(COLORREF color, int iTrend)
{
...
COLORREF CGraphCtrl::GetPlotColor(int iTrend)
{
...
void CGraphCtrl::InvalidateCtrl(bool deleteGraph)
{
	int i, j, GridPos;
	int nCharacters;
	CPen *oldPen;
	...
	COLORREF crLabelBk;
	COLORREF crLabelFg;
	bool bStraightGraphs = true;//false;//true;
...
//						 OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH, _T("MS Shell Dlg"));
	yUnitFont.CreateFont(15, 0, 900, 900, FW_NORMAL, FALSE, FALSE, 0, DEFAULT_CHARSET,
						 OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH, _T("@궁서체"));	// 권영기
    ...
    if (m_rectPlot.Height() / (m_nYGrids + 1) >= 14) {
	    for (j = 1; j < (m_nYGrids + 1); j++) {
		    GridPos = m_rectPlot.Height() * j / (m_nYGrids + 1) + m_rectPlot.top;
    	    strTemp.Format(_T("%.*lf"), m_nYDecimals, m_PlotData[0].dLowerLimit + (m_PlotData[0].dUpperLimit - m_PlotData[0].dLowerLimit) * (m_nYGrids - j + 1) / (m_nYGrids + 1));
    	    m_dcGrid.TextOut(m_rectPlot.left - 4, GridPos - 7, strTemp);
        }
    } else {
	    strTemp.Format(_T("%.*lf"), m_nYDecimals, m_PlotData[0].dLowerLimit + (m_PlotData[0].dUpperLimit - m_PlotData[0].dLowerLimit) / 2); 
	...
	CRect rText(0,0,0,0);
//	CRect rText(0,0,100,210); // 변경확인안됨.
	...
	int xpos = m_rectPlot.left + 50;//2;
//	int ypos = m_rectPlot.bottom + 3;
	int ypos = m_rectPlot.top-20;// 1;
...
//		m_dcGrid.TextOut(xpos + 12, ypos, m_PlotData[i].LegendLabel);
		m_dcGrid.TextOut(xpos + 40, ypos, m_PlotData[i].LegendLabel);
//		xpos += 12 + sizeLabel.cx + 12;
		xpos += 100 + sizeLabel.cx + 12;		// 그래프 종류 타이틀 2개 위치값
	}
...
void CGraphCtrl::AppendPoints(double dNewPoint[], bool bInvalidate, bool bAdd2List, bool bUseTrendRatio)
{
	...
	//		m_PlotData[iTrend].lstPoints.AddHead(m_PlotData[iTrend].dPreviousPosition);
...
// for valid points
//
void CGraphCtrl::AppendEmptyPoints(double dNewPoint[], bool bInvalidate, bool bAdd2List, bool bUseTrendRatio)
{
	int iTrend, currY;
...
/*
	static int i=0;
	printf("CGraphCtrl onPaint <%d>\n",i++);
*/
}

void CGraphCtrl::DrawPoint()
{
...
void CGraphCtrl::OnSysColorChange()
{
	if (m_bitmapOldGrid.m_hObject)
		m_dcGrid.SelectObject(m_bitmapOldGrid.Detach());
